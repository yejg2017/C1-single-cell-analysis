...
####  density method
```{r}
mouse.lognorm<-data.frame(FetchData(all.pbmc,vars.all =all.pbmc@var.genes))
mouse.lognorm$cell.size<-all.sample.size
mouse.lognorm$cell.group<-all.sample.group
mouse.lognorm.melt<-melt(mouse.lognorm)
```

```{r}
ggplot(data = mouse.lognorm.melt,aes(x=value))+
