...
batch.normalise.comBat <- function(counts, batch.groups, max.val=6)
{
  ...
  correct.data = ComBat(counts,batch.ids, prior.plots=FALSE, par.prior=T,mean.only =FALSE)
...
Simplify_Select<-function(data,seq.len=32,id=2,rc=2){
  
  #  seq.len: for quantile function,how many probs len to use
  #  id: afther quantile,select the id th for selection
  #  rc: row:1 or column:2
  data<-data[which(unlist(apply(data,1,sum)>0)),]

  each.data<-unlist(apply(data,rc,sum))
  seq.min<-as.numeric(quantile(each.data,probs = seq(0,1,length.out = seq.len))[id])
  seq.max<-as.numeric(quantile(each.data,probs = seq(0,1,length.out = seq.len))[seq.len-id-1])
  if(rc==2){
      data<-data[,which(each.data>seq.min&each.data<seq.max)]
      return(data)
  }
  if(rc==1){
      data<-data[which(each.data>seq.min&each.data<seq.max),]
...
DESeq_SeuratObj<-function(X,cells.1=NULL,cells.2=NULL,min.cells=0,min.genes=0,condition=NULL,
                          seq.len=32,id=2,FindVar=TRUE,condition.name=NULL,simple.rm=TRUE,DESq=FALSE){
  ...
  if(simple.rm){
    X<-Simplify_Select(X,seq.len = seq.len,id=id)
  }else{
    X<-X
  }
...
KS.test.comb<-function(object,cell_size='6um',genes=NULL){
  # object :Seurat object
  # genes:the genes' value s use to test
  # table:contain the group and cell size message
  sample.group<-unlist(lapply(object@cell.names,function(x)return(str_split(x,'_')[[1]][1])))
  sample.size<-unlist(lapply(object@cell.names,function(x)return(str_split(x,'_')[[1]][2])))
  #sample.size<-ifelse(sample.size=='6um','<6um',ifelse(sample.size=='10um','5-10um','10-17um'))
  if(is.null(genes)){
    lognorm<-data.frame(FetchData(object,vars.all =object@var.genes))
  }else{
    lognorm<-data.frame(FetchData(object,vars.all =genes[genes%in%rownames(object@raw.data)]))
  }
  #  melt data
  lognorm$cell.size<-sample.size
  lognorm$cell.group<-sample.group
  lognorm.melt<-melt(lognorm)
  
  #  create table
  size.group.table<-as.data.frame(as.matrix(table(sample.group,sample.size)))
  size.group.table<-size.group.table[size.group.table$Freq>0,]
  
  group<-as.character(size.group.table$sample.group)[as.character(size.group.table$sample.size)==cell_size]
  value.list<-list()
  for(g in group){
    idxg<-which(lognorm.melt$cell.group==g)
    idxc<-which(lognorm.melt$cell.size==cell_size)
    idx<-intersect(idxg,idxc)
    value<-list(lognorm.melt$value[idx])
    names(value)<-paste(cell_size,'_',g,sep = '')
  ...
  for(i in 1:(m-1)){
    for(j in (i+1):m){
      ks<-ks.test(x=value.list[[i]],y=value.list[[j]],alternative = 'two.sided')
      ks.p<-ks$p.value
      res<-list(ks.p)
      names(res)<-paste(str_split(names(value.list[i]),'_')[[1]][2],
                        '_',str_split(names(value.list[j]),'_')[[1]][2],sep = '')
      result<-c(result,res)
    }
  }
  result<-t(data.frame(result))
  colnames(result)<-paste(cell_size,'_','p.value',sep = '')
  return(result)
}

# Note: The function iLOO requires an input matrix, which represents a matrix of read counts for a 
# treatment or homogeneous group containing  samples and features. iLOO returns a matrix, where only
# the outlier read counts are present (all non-outlier values have been NA’ed). 


# https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4454687/
LOO <- function(sub) {
...
#  Anova analysis
Anova.gene<-function(data,gene,tuk.which=NULL,plot.aov=TRUE,plot.tuk=TRUE,inter=TRUE){
  # data : a melt data(use melt function)(cell.size cell.sample variable     value)
  # inter: whether consider inter relation between effect factors
  genes.data<-data[data$variable==gene,]
  
  # fit anova model
  if(inter){
    aov.gene<-aov(value~cell.size*cell.sample,data=genes.data)
  }else{
    aov.gene<-aov(value~cell.size+cell.sample,data=genes.data)
  }
  if(plot.aov){
    par(mfrow=c(2,2))
    plot(aov.gene,sub=gene)
  }
  if(is.null(tuk.which)){
    tuk<-TukeyHSD(aov.gene)
  }else{
    tuk<-TukeyHSD(aov.gene,which=tuk.which)
  }
  if(plot.tuk){
    plot(tuk,col='red',sub=gene)
  }
...
#  Kruskal-Wallis test for specify gene in all conditions
KruskalTest.gene<-function(data,gene){
  # data : a melt data(use melt function)(cell.size cell.sample variable     value)
  # gene: specify gene to use for test
  genes.data<-data[data$variable==gene,]
  res.cell.size<-kruskal.test(value~cell.size,data=genes.data)
  res.cell.sample<-kruskal.test(value~cell.sample,data=genes.data)
  result<-data.frame(chisq=c(as.numeric(res.cell.sample$statistic),as.numeric(res.cell.size$statistic)),
                     p.value=c(res.cell.sample$p.value,res.cell.size$p.value))
  rownames(result)<-paste(gene,c('cell.sample','cell.size'),sep='_')
  return(t(result))
}

#  QC method
QC.V1<-function(data,axis=2,thrx=0.3){
  # data:G*N,G genes,N sample genecount matrix or dataframe
  
  caculate<-function(vec){
    n<-length(vec)
    #i=0
    #for(x in vec){
    #  if(x!=0)
  ...
  if(axis==1){# select genes
    gene.names<-rownames(data)
    idx<-unlist(apply(data,axis,caculate))
    return(gene.names[idx])
  }
...
ComVariable<-function(tpm.data,rdata,genes){
  genes<-genes[genes%in%rownames(tpm.data)]
  cdata<-data.frame()
  for(gene in genes){
    gf<-as.numeric(tpm.data[gene,])
    var<-rep(gene,dim(tpm.data)[2])
    data<-rdata
    data$variable<-var
    data$feature<-gf
...
# melt data
SFC<-function(data,cell.size){
  
  group<-unique(as.character(data$cell.sample)[as.character(data$cell.size)==cell.size])
  value.list<-list()
  d<-data[data$cell.size==cell.size,]
  for(g in group){
    idx<-which(d$cell.sample==g)
    #idxc<-which(data$cell.size==cell.size)
    #idx<-intersect(idxg,idxc)
    value<-list(d$value[idx])
    names(value)<-paste(cell.size,'_',g,sep = '')
    value.list<-c(value.list,value)
  }
  ...
  result<-t(data.frame(result))
  colnames(result)<-paste(cell.size,'_','p.value',sep = '')
  return(result)
}
...
# 查看芯片细胞表达比率
Exp.Group<-function(Count.data,batch,batches,ratio=0.5,Exp.r=0.001){
  #Count.data: the gene count raw data
  # batch: which batch to check
  # batches: the all batches information
  # ratio : the criterion to decide whether drop this batch
  idx<-batches%in%batch
  batch.data<-Count.data[,idx]
  batch.idx<-apply(batch.data,2,function(x){
    i<-length(which(x!=0))
    n<-length(x)
    if(i/n>Exp.r){
      return(TRUE)
  ...
  n.batch<-sum(batch.idx)
  batch.r<-n.batch/dim(batch.data)[1]
  print(paste("The batch:",batch," Expression ratio is:",batch.r))
  if(batch.r>ratio){
    print(paste("Retente the batch:",batch))
    return(TRUE)
  }else{
    print(paste("Drop the batch:",batch))
    return(FALSE)
  }
}
...
# Group pearson boxplot
Batch.Cor <- function(data,batch,batches) {
  cell_matrix <- as.matrix(data[, batches == batch])
...
select.genes<-function(sentence){
  ens<-str_extract(sentence,'ENSMFAG\\d+')
  genel<-str_extract_all(sentence,'[A-Z0-9]{1,}')[[1]]
  gene<-c()
  
  for(S in genel){
    s<-str_split(S,'')[[1]]
    if((length(s)>10)|(!str_detect(s,'[A-Za-z]'))){
      next
    }else{
      gene<-c(gene,S)
    }
  }
  return(data.frame(esmeble=ens,gene=unique(gene)))
}


# Function to find control genes by finding the closest genes in terms of expression level and % of the time we observe it
...
# 根据marker 基因在不同cluster中的score值确定cluster的类别名称
DecideScore<-function(object,gene.list,control.genes.per.gene=10){
  raw.data<-as.matrix(object@raw.data)
  ident<-object@ident
  all.genes<-rownames(raw.data)
  gene.list<-gene.list[gene.list%in%all.genes]
  
  #Tpm<-data.frame(log(1+tpm(raw.data)))
  parall.cal<-function(idx){
    idx.raw<-raw.data[,which(ident==idx)]
    Tpm<-data.frame(log(1+tpm(idx.raw)))
    control_genes = get_controls(counts = Tpm, gene.list = gene.list, verbose=F)
    score = Matrix::colMeans(Tpm[gene.list,]) - Matrix::colMeans(Tpm[control_genes,])
    m_score<-mean(score)
    return(m_score)
  }
  
  scores<-lapply(unique(ident),function(i){
    return(data.frame(id=i,score=parall.cal(i)))
  })
  scores<-data.frame(bind_rows(scores))
  id<-scores$id[which.max(scores$score)]
  return(id)
  #return(scores)
}
...
##  Specify heatmap
SpecifyHp<-function(object,condition,rm.ident=NULL,n=10){
  if(is.null(rm.ident)){
    subobject<-SubsetData(object = object,
                          cells.use =object@cell.names[object@meta.data$condition==condition])
  }else{
    subobject<-SubsetData(object = object,
                        cells.use =object@cell.names[object@meta.data$condition==condition&
                                                       object@ident!=rm.ident]
                        )
  }
  
  #print(subobject)
  markers <- FindAllMarkers(object =subobject, only.pos = TRUE, min.pct = 0.25, thresh.use = 0.25,
                            print.bar = FALSE)
  markers %>% group_by(cluster) %>% top_n(n, avg_logFC) ->top10
  
  gene.h<-top10$gene[!str_detect(top10$gene,'-')]
  print(DoHeatmap(subobject,genes.use =gene.h,
            slim.col.label = TRUE,group.label.rot = TRUE,
            cex.row = 10,title =condition))
  
}


## Caculate cell type tpm
SignatureScore<-function(object,ident,gene.list,control.gene.nums=10){
  raw.data<-as.matrix(object@raw.data[,object@ident==ident])
  gene.list<-gene.list[gene.list%in%rownames(object@scale.data)]
  Tpm<-data.frame(log(1+tpm(raw.data)))
  control_genes<-get_controls(Tpm,gene.list = gene.list,control.genes.per.gene = control.gene.nums)
...
SEV.genes<-function(object,genes.fit,reduction.use='tsne',orig.ident=NULL,new.ident=NULL){
  if(is.null(genes.fit)){
    stop('gene.fit can not be null...')
  }
  x<-AddSmoothedScore(object = object,genes.fit = genes.fit,reduction.use = reduction.use)
  SEV.t<-data.frame(t(x@imputed))
  rownames(SEV.t)<-1:dim(SEV.t)[1]
  
  cat('Add messages...\n ')
  if(is.null(orig.ident)){
    print('orig.ident is null,it is invalid!!! ')
  }else{
    species<-ifelse(str_detect(orig.ident,'mc'),'mouse','human')
  }
  
  SEV.t$ident<-orig.ident
  SEV.t$species<-species
  
  if(!is.null(new.ident)){
    SEV.t$ident<-new.ident
  }
  return(SEV.t)
}


### Find the highest expression gene in specify cluster
GeneClusterTest<-function(Data,cluster.t,cluster.c1,cluster.c2,clusters,genes.list=NULL,p.value=0.05,ntop=30){

  save.genes<-c()
  
  if(is.null(genes.list)){
    genes.list<-rownames(Data)
  }
  for(gene in genes.list){
    dt<-as.numeric(Data[gene,clusters==cluster.t])# target cluster
    dc1<-as.numeric(Data[gene,clusters==cluster.c1])
    dc2<-as.numeric(Data[gene,clusters==cluster.c2])
    
    p.t_c1<-wilcox.test(dt,dc1,alternative = 'greater')$p.value
    p.t_c2<-wilcox.test(dt,dc2,alternative = 'greater')$p.value
    
    if((p.t_c1<p.value)&(p.t_c2<p.value)){
      save.genes<-c(save.genes,gene)
    }
  }
  #return(save.genes)
  x<-Data[save.genes,clusters==cluster.t]
  xrowM<-data.frame(gene.m=rowMeans(x))
  top.genes<-save.genes[order(xrowM$gene.m,decreasing = T)][1:ntop]
  return(top.genes)
}


## add genes 
Add.genes<-function(gene2move,gene.target){
  for(gene in gene2move){
    if(!gene%in%gene.target){
       gene.target<-union(gene.target,gene)
    }
  }
  return(gene.target)
}
